...
export class PlaylistService {
  ...
  createPlaylist(name: string, description: string = '', initialMedia?: MediaType | MediaType[]): string {
  ...
  addToPlaylist(playlistId: string, media: MediaType): void {
    this.playlists.update(playlists => playlists.map(p => {
      if (p.id === playlistId && !p.items.some(item => item.id === media.id)) {
  ...
  removeFromPlaylist(playlistId: string, mediaId: number): void {
    this.playlists.update(playlists => playlists.map(p => {
      if (p.id === playlistId) {
        return { ...p, items: p.items.filter(item => item.id !== mediaId), updatedAt: Date.now() };
      }
      return p;
    }));
  }
  ...
  isMediaInPlaylist(playlistId: string, mediaId: number): boolean {
    ...
    return playlist ? playlist.items.some(item => item.id === mediaId) : false;
  }
  
  isMediaInAnyPlaylist(mediaId: number): boolean {
    return this.playlists().some(p => p.items.some(item => item.id === mediaId));
  }

  getPlaylistsForMedia(mediaId: number): Playlist[] {
    return this.playlists().filter(p => p.items.some(item => item.id === mediaId));
  }
}