...
export class HistoryItemCardComponent {
  ...
  isOnWatchlist = computed(() =>
    this.watchlistService.isOnWatchlist(this.item().media.id)
  );
  isInPlaylist = computed(() =>
    this.playlistService.isMediaInAnyPlaylist(this.item().media.id)
  );

  progress = computed(() => {
    const historyItem = this.item();
    const progressId = historyItem.episode
      ? historyItem.episode.id
      : historyItem.media.id;
