...
export class VideoPlayerComponent implements OnInit, OnDestroy {
  ...
  // Track whether we've requested the next episode details to avoid redundant network requests
  nextEpisodeRequested = signal(false);
  ...
  // Performance: Throttle counter for progress updates
  private lastSignalUpdateTime = 0;

  // Current playback time (exposed to template, throttled update)
  ...
  showNextEpisodeButton = computed(() => {
    ...
    // 4. AND either:
    //    a. Progress is > 80% (pre-show button)
  ...
  playerUrl = computed<string | null>(() => {
    ...
    let resumeTime = 0;
    const currentTime = untracked(() => this.currentPlaybackTime());
  ...
  constructor() {
    ...
    effect((onCleanup) => {
      ...
      if (isActualMediaChange || isPlayerChange) {
        ...
        // Reset next-episode cache/request flag so we reload details for the new media/episode
        this.nextEpisode.set(null);
        this.nextEpisodeRequested.set(false);
  ...
  private getMediaKey(params: any): string {
    return `${params.mediaType}:${params.id}:${params.season ?? ""}:${
      params.episode ?? ""
    }`;
  }
  ...
  private handlePlaybackProgress(
    ...
    // Safety: Ignore invalid duration or infinite progress which causes premature completion/auto-next
    if (!duration || duration <= 0 || !isFinite(progressPercent)) {
      return;
    }
    ...
    const now = Date.now();
    const shouldUpdateSignals = now - this.lastSignalUpdateTime >= 1000;

    // 1. Update Signals (Throttled)
    // We update signals roughly every second to avoid excessive CD cycles
    if (shouldUpdateSignals || progressPercent >= 100) {
      this.currentPlaybackTime.set(currentTime);
      this.currentProgressPercent.set(progressPercent);
      this.lastSignalUpdateTime = now;

      // Handle navigation lock release check throttled as well
      if (currentTime >= this.MIN_PLAYBACK_FOR_RESET && this.isNavigating()) {
        ...
        const timeSinceNav = now - this.navigationStartTime;
    ...
    // Update persistence service (it throttles internally as well)
    // We only update the service when signals update to avoid excessive writes/signals downstream
    if (!isFailedResume && (shouldUpdateSignals || progressPercent >= 100)) {
    ...
    // Preload next episode recommendation
    if (
      media.media_type === "tv" &&
    ...
    // Trigger auto next check (uses raw values for precision)
  ...
  private checkAndTriggerAutoNext(
    ...
    // Ensure next episode details are fetched early so the Up Next card has its thumbnail/name
    // when the pre-show appears (progress >= 80%). We avoid duplicate requests with nextEpisodeRequested.
    if (
      media.media_type === "tv" &&
      !this.nextEpisode() &&
      !this.nextEpisodeRequested() &&
      hasNextItem &&
      progressPercent >= 80
    ) {
      this.nextEpisodeRequested.set(true);
      this.fetchNextEpisode(media as TvShowDetails, this.currentEpisode())
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe((next) => {
          this.nextEpisode.set(next);
        });
    }
  ...
  private startAutoNextCountdown(media: MovieDetails | TvShowDetails): void {
    ...
    // Ensure we have next episode details available immediately when countdown starts
    if (
      media.media_type === "tv" &&
      !this.nextEpisode() &&
      !this.nextEpisodeRequested() &&
  ...
  private canProcessEpisodeChange(eventName?: string): boolean {
    ...
    return (
      ...
      !this.isNavigating() &&
      (this.currentPlaybackTime() >= 5 || nonRoutineEvent)
    );
  }
